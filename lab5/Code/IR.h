...
DEF_VECTOR(IR_var)
extern IR_var ir_var_generator();     // 获取**新**变量编号
...
struct IR_stmt_virtualTable {
  ...
  IR_var (*get_def)(
      IR_stmt *stmt); // def 最多只有一个, 不存在则返回 IR_VAR_NONE
...
typedef struct IR_function {
  ...
  Map_IR_block_ptr_List_ptr_IR_block_ptr blk_pred,
      blk_succ; // Block 指针 -> List<前驱后继Block> 的指针
} IR_function, *IR_function_ptr;
DEF_VECTOR(IR_function_ptr)
extern void IR_function_init(IR_function *func, const char *func_name);
extern IR_var IR_function_insert_dec(IR_function *func, IR_var var,
                                     IR_DEC_size_t dec_size);
...
typedef enum { IR_OP_ADD, IR_OP_SUB, IR_OP_MUL, IR_OP_DIV } IR_OP_TYPE;
typedef struct {
  CLASS_IR_stmt IR_OP_TYPE op;
  IR_var rd;
  union {
    IR_val use_vec[2];
    struct {
...
extern void IR_op_stmt_init(IR_op_stmt *op_stmt, IR_OP_TYPE op, IR_var rd,
                            IR_val rs1, IR_val rs2);

typedef struct {
  CLASS_IR_stmt IR_var rd;
  union {
    IR_val use_vec[1];
    struct {
      IR_val rs;
...
extern void IR_assign_stmt_init(IR_assign_stmt *assign_stmt, IR_var rd,
                                IR_val rs);
...
extern void IR_load_stmt_init(IR_load_stmt *load_stmt, IR_var rd,
                              IR_val rs_addr);
...
extern void IR_store_stmt_init(IR_store_stmt *store_stmt, IR_val rd_addr,
                               IR_val rs);
...
typedef struct {
  CLASS_IR_stmt IR_RELOP_TYPE relop;
  union {
    IR_val use_vec[2];
    struct {
      IR_val rs1, rs2;
...
extern void IR_if_stmt_init(IR_if_stmt *if_stmt, IR_RELOP_TYPE relop,
                            IR_val rs1, IR_val rs2, IR_label true_label,
                            IR_label false_label);
extern void IR_if_stmt_flip(IR_if_stmt *if_stmt);

typedef struct {
  CLASS_IR_stmt IR_label label;
  IR_block *blk;
} IR_goto_stmt;
...
extern void IR_call_stmt_init(IR_call_stmt *call_stmt, IR_var rd,
                              const char *func_name, unsigned argc,
                              IR_val *argv);
